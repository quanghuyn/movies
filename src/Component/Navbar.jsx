...
function Navbar(props) {
        ...
        const pathname = location.pathname.slice(0,16)
    ...
    return (
        <div className={pathname ==='/movies-details/' ? "navbarLocation" : 'navbar'} >
     {/* PC CSS */}
                <NavLink to={'/'} className =' flex font-bold text-2xl items-center ml-14 w-3/12 z-50 max-lg:hidden '> Cinemax </NavLink>
                ...
                <div className=' flex items-center pt-2 ml-16  w-2/12 '>
                    <input type="text" placeholder='Search' className='max-sm:hidden  max-lg:ml-20  pl-3 pb-1 placeholder:text-fontnormal  focus:outline-none text-opacity-60 bg-main-dark-bg h-7 rounded-full w-30  border border-fontnormal' />
                    <span>
                    <svg className='ml-3 max-lg:hidden' width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
...
{/* Mobi & Tab css */}
                <Link to={'/'}>
                    <img className='absolute w-32 left-0 -top-3 lg:hidden '  src={logo} alt="" />
                </Link>
                {/* menu icon */}
                <div className='absolute right-0 top-3 lg:hidden'>
                    <svg onClick={e => handlClose(e)} className="w-10 h-10 mr-4 " xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" >
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
                    </svg>
                </div>

                <div  className={` w-full absolute z-30 lg:hidden bg-main-dark-bg ${open ? 'hidden': null} `}>
                <div onClick={() => handlClose()} className='cursor-pointer'>
                     <svg  id='closesvg' xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="absolute  w-6 h-6 right-0 mr-6 mt-3   ">
                            ...
                            {data.map((item,index) => {
                                return (
                                    ...
                                    className= 'flex flex-col items-center'
                                        >
                                        <NavLink
                                            className= {({isActive}) => isActive ? 'linkActiveRs' : 'link' }
                                            to={`/${item.to}`}>
                                            {item.name}
                                        </NavLink>
                                    </div>
